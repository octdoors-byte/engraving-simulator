...
**Purpose（本ツールが解決すること）**
- 本ツールは「デザインを作る」ためではなく、刻印工程に投入する前の「配置確認・確定（合意形成）」を行う。
...
- デザイン提案・自動レイアウト
- 色変更、装飾（影・縁・フィルタ等）
- 画像編集（切り抜き、合成、レタッチ、トレース、ベクタ化等）
- 入稿用データ作成（Illustrator形式必須の工程）
- カラーバリエーションの厳密再現（表示は参考）

### 1.5.2 入力データの前提（Customer Asset First）
- お客様は既に支給データ（ロゴ原稿・設計図・画像）を持っている前提とする。
- 本ツールは「支給データを刻印工程で使える状態に当てはめて確定する」ことが役割であり、支給データ作成は責務外とする。

### 1.5.3 刻印工程に特化した制約（Manufacturing Constraints）
- 刻印の物理制約を最優先する。
- 刻印可能範囲（Bounding Box）を必ず表示し、範囲外への配置は禁止または明確に警告する。
- 出力は「見た目」より「制作指示として再現可能」であることを優先する。
- 刻印は不可逆であるため、曖昧さを残さないUI/状態を採用する。
- 最終確定操作（Confirm）を必須とし、確定前後で状態を分離する。
- 確定後は、位置・サイズ・入力データのハッシュ等を含めて追跡可能にする（既存仕様に合わせてON/OFF）。

### 1.5.4 表示の取り扱い（Color & Preview Disclaimer）
- 商品画像や色味の再現は参考表示であり、厳密な色再現を目的としない。
- プレビューは「刻印位置・サイズ・余白バランス確認」のための表示であり、素材差・光源差・モニタ差により実物と差が出る可能性がある。

### 1.5.5 成功条件（Done Definitionに追記）
- お客様がツール上で配置を確定でき、追加のメール確認なしで制作工程に進められること。
- 確定結果として、以下が取得できること。
  - 出力画像（PNG）および必要に応じてPDF
  - 位置・サイズ情報（数値）
  - テンプレートID/デザインID等の参照情報（既存仕様に準拠）

### 1.5.6 仕様書内の名称（文書上の推奨）
- 文書上は「デザインシミュレーター」ではなく、以下のいずれかを推奨する。
  - 刻印配置確認ツール
  - 刻印前確認シミュレーター
  - ロゴ配置確定ツール
