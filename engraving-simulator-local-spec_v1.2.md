# デザインシミュレーター 仕様書（最新版に統合）
spec_id: engraving-simulator-local-v1.2
対象: ローカル環境 (Vite + React + TypeScript + Tailwind)
更新日: 2026-01-09
方針: md_kakunin.md の内容に合わせて統合
...
- 状態は draft/tested/published/archive を切り替えられる
- 公開前テンプレは /sim で利用不可

### 2.5 テンプレート運用ルール（公開中/テスト済み/下書き/アーカイブ）
- status は4種類のみ: 下書き / テスト済み / 公開中 / アーカイブ
- お客様の新規作成で表示・選択できるのは 公開中 のみ
- 下書き: 作成途中。管理者のみ閲覧・編集可能
- テスト済み: 動作確認済み。管理者のみ閲覧可能。公開中に切り替えればお客様に出せる
- 公開中: お客様の新規作成で選択可能
- アーカイブ: お客様には表示しない。過去デザインの再表示・PDF再ダウンロードのために保持
- 仕様変更時は既存テンプレを上書きしない
  - 新規テンプレを作成して下書き/テスト済み
  - 公開するものだけ公開中
  - 旧テンプレはアーカイブ
- design_id が1件でも紐づくテンプレは削除不可。削除の代替はアーカイブ
- アーカイブは編集不可（刻印枠/基準位置/初期位置/サイズ条件など結果に影響する項目は凍結）
- ステータス遷移（許可）
  - 下書き → テスト済み
  - テスト済み → 公開中
  - 公開中 → アーカイブ
...
- status: draft/tested/published/archive（画面表示: 下書き/テスト済み/公開中/アーカイブ）
- archive は編集不可（凍結）
...
- 刻印用PDFは発行時に作成されるが、履歴画面ではプレビュー表示しない
...
- 状態の切り替え（下書き / テスト済み / 公開中 / アーカイブ）
- テスト / スマホ / PC 表示の確認
- status は下書き/テスト済み/公開中/アーカイブを識別できる
- ステータス遷移は運用ルールに従う（アーカイブからの復帰は禁止）
- カテゴリ/備考（お客様表示用）は管理画面で編集する
...
## 14.5 公開テンプレート一覧（/top）
- 公開中（published）のみ表示
- 表示列: 表示名 / カテゴリ / 備考（お客様表示用） / 用紙 / テンプレキー / お客様公開URL
- 備考は一覧でダブルクリックして編集できる
- 列の並び替え、表示/非表示、列幅調整ができる
- 行間と文字サイズをスライダーで調整できる
...
---
## Best Practices：PDF(mm)整合性・最小サイズ注意・解像度警告

### ① mmがPDF上で常に一致するか（ズーム/画面サイズ影響の排除）
#### 成功条件（必須）
- 同一デザインで、以下操作を行っても、PDFに記載する `X/Y/W/H (mm)` が常に一致すること:
  - ブラウザズーム: 80% / 100% / 125%
  - 画面サイズ変更（ウィンドウリサイズ）
- 一致しない（ズレる）事象が発生する場合は、下記「対策（必須）」の方式へ強制的に寄せること。

#### 対策（必須: 実装方針の固定）
- 真（正）の座標系は常に mm とする。
  - デザインデータ（ロゴの位置/サイズ、枠など）を mm で保持する（保存もmm）。
  - UIは「表示のためにmm→pxへ変換して描画」するだけにする。
  - ブラウザズームや devicePixelRatio の変化は UI変換層で吸収し、保存値(mm)やPDF値(mm)は影響を受けない設計に固定する。

#### PDF出力の単位変換（固定）
- PDF内部単位は point(pt) を使用し、以下で変換する（絶対にpxを介さない）:
  - `pt = mm * 72 / 25.4`
  - `mm = pt * 25.4 / 72`
- PDFに記載する `X/Y/W/H(mm)` は 保存しているmm値そのもの を出す（UI計算由来のpxから再計算しない）。

---

### ② 最小サイズの注意（注意POP表示のみ: 出力はブロックしない）
#### 方針（固定）
- ロゴの最小サイズ `minW_mm` / `minH_mm` をチェックする。
- 最小未満の場合は 注意POP（警告）を表示する。
- PDF出力は許可する（ボタン無効化・強制エラー表示はしない）。
- 可能なら出力前チェック表示（推奨）:
  - `サイズ: OK / NG` を明示（NGでも出力自体は可能）

#### テンプレ項目化（推奨）
- テンプレートに以下を追加できるなら項目化する:
  - `logo_min_w_mm`
  - `logo_min_h_mm`
- テンプレに無い場合はデフォルト値を固定で持つ（例: どちらも 5mm など ※値は仕様側で確定）。

#### 判定式（固定）
- `isBelowMin = (logo_w_mm < minW_mm) OR (logo_h_mm < minH_mm)`
- `isSizeOK = NOT isBelowMin`

#### 注意POP文言（固定: やわらかめ）
- タイトル例: `サイズが小さめです`
- 本文例:
  - `設定されたサイズが推奨の最小値を下回っています。仕上がりが見えにくくなる可能性があります。`
  - `必要に応じてサイズを大きくしてご確認ください。`
- 併記（任意）:
  - `推奨最小: W {minW_mm}mm / H {minH_mm}mm`

---

### ③ ロゴ解像度の警告（出力は許可）
#### 目的（推奨）
- 取り込み画像の元解像度(px) と、最終出力サイズ(mm) から、拡大率/実効解像度を推定し、荒くなる可能性が高い場合は警告する。

#### 計算（固定）
- `width_in = logo_w_mm / 25.4`
- `height_in = logo_h_mm / 25.4`
- `dpi_x = img_px_w / width_in`
- `dpi_y = img_px_h / height_in`
- `effective_dpi = MIN(dpi_x, dpi_y)`

#### 警告条件（推奨: 閾値固定）
- `effective_dpi < 150`: 強い警告（荒さが目立つ可能性が高い）
- `150 <= effective_dpi < 300`: 注意（用途によっては許容）
- `>= 300`: OK（推奨水準）
- 警告は 出力をブロックしない。

#### 警告文（固定）
- 「ロゴ画像の解像度が低いため、仕上がりが粗く見える可能性があります。」
- 「推奨: より高解像度のロゴをご用意ください。」

---

### 実装上の禁止事項（事故防止）
- mm値を「UI上のpx座標」から逆算して保存・PDF出力しない。
- ブラウザズームやdevicePixelRatioに依存する値を、最終(mm)の根拠にしない。
...
---
## 19. 更新履歴（2026-01-13）
- /top は公開中（published）のみ表示に変更
- /top に「カテゴリ」「備考（お客様表示用）」を追加
- /top の列並び替え/列幅調整/表示切替/行間/文字サイズの調整を追加
- /admin/designs は確認用PDFのみプレビュー・ダウンロード
